...
Puppet::Type.type(:mysql_user).provide(:mysql, :parent => Puppet::Provider::Mysql) do
  ...
  def create
    ...
    if (((mysqld_type == "mysql" or mysqld_type == "percona") and Puppet::Util::Package.versioncmp(mysqld_version, '5.7.6') >= 0) or
      ...
      mysql([defaults_file, '-e', "ALTER USER '#{merged_name}' REQUIRE #{merged_tls_options}"].compact)
    else
      mysql([defaults_file, '-e', "GRANT USAGE ON *.* TO '#{merged_name}' REQUIRE #{merged_tls_options}"].compact)
    end
